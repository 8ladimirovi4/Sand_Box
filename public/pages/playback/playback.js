...
window.addEventListener("beforeunload", async function (event) {
  try {
    await fetch(`https://localhost:3001/playback/stop_ffmpeg_process`,{
      method: 'POST',
      headers: {
...
fetch(`https://localhost:3001/playback/${id}/${camNo}/${startTime}/${endTime}`)

const isManifestExist = async() => {
  try{
   const res = await fetch(`https://localhost:3001/playback/is_file_loaded`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
const loadVideo = async() => {
  if (Hls.isSupported()) {
    const hls = new Hls();
    hls.loadSource(
      `https://localhost:3001/playback_data/${id}/out.m3u8`
    );
    hls.attachMedia(VIDEO);
    hls.on(Hls.Events.MANIFEST_PARSED, function () {});
  } else if (VIDEO.canPlayType("application/vnd.apple.mpegurl")) {
    VIDEO.src = `https://localhost:3001/playback_data/${id}/out.m3u8`;
